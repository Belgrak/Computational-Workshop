...
def get_p_n(x, d, n):
    if n == 0:
        return d[0][1]
    return get_p_n(x, d, n - 1) + \
        (d[n][1] - d[n - 1][1]) / (d[n][0] - d[n - 1][0]) * math.prod([x - i[0] for i in d[:n]])
...
if __name__ == '__main__':
    ...
    while com != "exit":
        run_task()
        com = input("Нажмите Enter, чтобы продолжить или введите exit, чтобы завершить программу\n")